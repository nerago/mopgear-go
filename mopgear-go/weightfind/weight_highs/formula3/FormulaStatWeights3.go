...
const (
	c_formula3HighDiff         = 1000.0
	c_formula3OutputPerInclude = -0.1
	c_formula3ScaleTarget      = 1.0
)
...
func (form *FormulaStatWeightProcess3) buildDataEquations() {
	for data := range util_collection.ForPointer(form.inputData) {
		form.buildDataEquationForInput(data)
	}
}

func (form *FormulaStatWeightProcess3) buildDataEquationForInput(data *weight_types.WeightInput) {
...
func (form *FormulaStatWeightProcess3) sampleIncludeToggleColumn() util_highs.ColumnIndex {
...
func (form *FormulaStatWeightProcess3) buildDataEquationForSim(stats *stats.StatBlock, simValue float64, simType stats.SimType, includeColumn util_highs.ColumnIndex) {
	...
	deviationAbsOutput := form.build.CreateColumnWithObjective(highs.Continuous, 0, c_formula3HighDiff, 1, form.objectiveEquationDiff, util_highs.DebugString{Text: "deviationAbsOutput"})

	matchSimValue.Add(deviationSigned, 1)
	form.build.AbsoluteValue_WithToggle(deviationSigned, deviationAbsOutput, includeColumn, c_formula3HighDiff)
...
func (form *FormulaStatWeightProcess3) reportExamples(weightExtended *weight_types.Weight2Extended) {
