...
func (grid2 *GridStatWeightProcess2) Run() (*util_async.FutureCancellable[weight_types.WeightResult2], error) {
	...
	return util_async.FutureCancellable_MapValue(solutionFuture, func(linearResult util_highs.LinearResult) weight_types.WeightResult2 {
		if solution, err := linearResult.GetSolutionAndSaveLog(grid2.printer); err == nil {
			grid2.printer.Println(solution.Status.String())
			grid2.build.DebugPrintColumns(solution, grid2.printer)

			weight := grid2.reportOutputWeightsGrid(solution)
