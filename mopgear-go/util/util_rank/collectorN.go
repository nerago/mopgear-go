...
func (collect *LowestCollectorN[T]) Offer(object *T, value float64) {
...
func (collect *collectorNInternal[T]) ResultsSeq() iter.Seq[*T] {
	return func(yield func(*T) bool) {
		for i := range collect.array {
			if !yield(collect.array[i].object) {
				return
			}
		}
	}
...
func (collect *collectorNInternal[T]) GetBest1() (*T, float64) {
	if len(collect.array) > 0 {
		bestEntry := collect.array[len(collect.array)-1]
		return bestEntry.object, bestEntry.value
	} else {
		return nil, 0
	}
}

func LowestCollectorN_OfChannel[T any](channel <-chan LowestCollectorN[T], expectNum int) iter.Seq[*T] {
...
func (collect *HighestCollectorN[T]) Offer(object *T, value float64) {
	if collect.size < collect.limit {
		entry := internalEntry[T]{object, value}
		collect.array = append(collect.array, entry)
		collect.sortContent()
...
func HighestCollectorN_OfChannel[T any](channel <-chan HighestCollectorN[T], expectNum int) iter.Seq[*T] {
