...
func StatBlock_Add_Into_Experiment(a, b, out *StatBlock) {
	a1 := archsimd.LoadUint32x8(a[0:8])
	a2 := archsimd.LoadUint32x4(a[8:12])
	b1 := archsimd.LoadUint32x8(b[0:8])
	b2 := archsimd.LoadUint32x4(b[8:12])
	s1 := a1.Add(b1)
	s2 := a2.Add(b2)
	s1.Store(out[0:8])
	s2.Store(out[8:12])
...
func StatBlock_AddAndSubtract_Into_Experiment(add1, add2, subtract, out *StatBlock) {
	a1 := archsimd.LoadUint32x8(add1[0:8])
	b1 := archsimd.LoadUint32x8(add2[0:8])
	c1 := archsimd.LoadUint32x8(subtract[0:8])
	s1 := a1.Add(b1).Sub(c1)
	s1.Store(out[0:8])

	a2 := archsimd.LoadUint32x4(add1[8:12])
	b2 := archsimd.LoadUint32x4(add2[8:12])
	c2 := archsimd.LoadUint32x4(subtract[8:12])
	s2 := a2.Add(b2).Sub(c2)
	s2.Store(out[8:12])
}

func StatBlock_Increment_Mutating_Experiment(mutate *StatBlock, other *StatBlock) {
	m1 := archsimd.LoadUint32x8(mutate[0:8])
	m2 := archsimd.LoadUint32x4(mutate[8:12])
	o1 := archsimd.LoadUint32x8(other[0:8])
	o2 := archsimd.LoadUint32x4(other[8:12])
	s1 := m1.Add(o1)
	s2 := m2.Add(o2)
	s1.Store(mutate[0:8])
	s2.Store(mutate[8:12])
}
...
func StatBlock_StatBlockFloat_MultiplyForTotalSum2(a *StatBlockFloat, b *StatBlock) float64 {
	a1 := archsimd.LoadFloat64x4(a[0:4])
	a2 := archsimd.LoadFloat64x4(a[4:8])
	a3 := archsimd.LoadFloat64x4(a[8:12])
	...
	b1 := archsimd.LoadInt32x4Array(p1).ConvertToFloat64()
	b2 := archsimd.LoadInt32x4Array(p2).ConvertToFloat64()
	b3 := archsimd.LoadInt32x4Array(p3).ConvertToFloat64()
	...
	parts := [4]float64{}
	total.StoreArray(&parts)

	return parts[0] + parts[1] + parts[2] + parts[3]
}

func StatBlock_StatBlockFloat_MultiplyForTotalSumZZZ(a *StatBlockFloat, b *StatBlock) float64 {
	a1 := archsimd.LoadFloat64x4(a[0:4]).ConvertToFloat32()
	a2 := archsimd.LoadFloat64x4(a[4:8]).ConvertToFloat32()
	a12 := archsimd.Float32x8{}
	a12.SetLo(a1)
	a12.SetHi(a2)
	a3 := archsimd.LoadFloat64x4(a[8:12]).ConvertToFloat32()

	//b1 := archsimd.LoadUint32x8(b[0:8]).ConvertToFloat32()
	//b2 := archsimd.LoadUint32x4(b[8:12]).ConvertToFloat32()
	...
	b1 := archsimd.LoadInt32x8Array(ptrB1Signed).ConvertToFloat32()
	b2 := archsimd.LoadInt32x4Array(ptrB2Signed).ConvertToFloat32()
