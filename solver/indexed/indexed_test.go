...
func TestMultiInt(t *testing.T) {
	...
	_ = mainLoop_multiThread_int(options, max.Uint64(), skip.Uint64(), util.TrackProgress_Nop(), model, peekRecord.Add)
...
func TestMultiBig(t *testing.T) {
	const targetCount = utiltest.TargetCountStandard
	peekRecord, options, model, max, skip := setup(targetCount)
	mainLoop_multiThread_big(options, max, skip, util.TrackProgress_Nop(), model, peekRecord.Add)
	utiltest.VerifyRecord(t, peekRecord, options, targetCount)
}
...
func TestSingleInt(t *testing.T) {
	const targetCount = utiltest.TargetCountStandard
	peekRecord, options, model, max, skip := setup(targetCount)
	mainLoop_singleThread_int(options, max.Uint64(), skip.Uint64(), util.TrackProgress_Nop(), model, peekRecord.Add)
	utiltest.VerifyRecord(t, peekRecord, options, targetCount)
}
...
func TestSingleBig(t *testing.T) {
	const targetCount = utiltest.TargetCountStandard
	peekRecord, options, model, max, skip := setup(targetCount)
	mainLoop_singleThread_big(options, max, skip, util.TrackProgress_Nop(), model, peekRecord.Add)
	utiltest.VerifyRecord(t, peekRecord, options, targetCount)
}
