...
func StatRequirementsGeneral_RetWideCap() *StatRequirementsGeneral {
	inst := &StatRequirementsGeneral{asMap: make(map[stats.StatType]util.HiLoUInt32, 2), hi: fullHighBlock()}
...
func StatRequirementsGeneral_ProtFlexibleParry_PlusAdditional(additional *stats.StatAndValue) *StatRequirementsGeneral {
	inst := &StatRequirementsGeneral{asMap: make(map[stats.StatType]util.HiLoUInt32, 3), hi: fullHighBlock()}
	inst.addRange(stats.Stat_Hit, TARGET_RATING_MELEE, TARGET_RATING_MELEE+DEFAULT_CAP_EXCEED)
...
func (inst *StatRequirementsGeneral) addRange(statType stats.StatType, lo uint32, hi uint32) {
	inst.asMap[statType] = util.HiLoUInt32{Lo: lo, Hi: hi}
	inst.lo[statType] = lo
	inst.hi[statType] = hi
}
...
func (inst *StatRequirementsGeneral) AsMap() map[stats.StatType]util.HiLoUInt32 {
	return inst.asMap
}
