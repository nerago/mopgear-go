...
type SelectiveGridStatWeightProcess struct {
	...
	inputDataIncludeToggles []util_highs.ColumnIndex
...
func (grid *SelectiveGridStatWeightProcess) Run(stopwatch *util.Stopwatch) *util_async.FutureCancellable[WeightResult] {
...
func (grid *SelectiveGridStatWeightProcess) createIncludeToggles() {
	...
	rowIncludeReasonableNumber := util_highs.ConstraintRow{}

	grid.inputDataIncludeToggles = make([]util_highs.ColumnIndex, len(grid.inputData))
	for i := range len(grid.inputData) {
		column := grid.build.CreateColumnBoolWithOutput(includeScore, util_highs.DebugString{Text: "input toggle " + strconv.Itoa(i)})
...
func (grid *SelectiveGridStatWeightProcess) prepareSample(statType stats.StatType, high, low *WeightInput, highInclude, lowInclude util_highs.ColumnIndex) {
	...
	for _, simType := range grid.requiredSims {
		...
		dataSample := selectiveGridDataSample{unitStatValue, [2]util_highs.ColumnIndex{highInclude, lowInclude}}
...
func (grid *SelectiveGridStatWeightProcess) unitValueCombinationAddToModel(baseUnitSample selectiveGridDataSample, detailWeightBase util_highs.ColumnIndex,
	thisUnitSample selectiveGridDataSample, thisDetailWeight util_highs.ColumnIndex, debugText string) {

	includeDataPointToggle := grid.build.CreateColumnBool(util_highs.DebugString{Text: "DATA TOGGLE " + debugText})
	...
	offsetSigned := grid.build.CreateColumnGeneral(highs.Continuous, util_highs.C_MinusInf, util_highs.C_PlusInf, util_highs.DebugString{Text: "OFFSET SIGNED " + debugText})
	weightRow := util_highs.ConstraintRow{}
	...
	// take absolute value
	offsetAbs := grid.build.CreateColumnWithOutput(highs.Continuous, 0, c_selGrid_formulaHigh, 1, util_highs.DebugString{Text: "OFFSET ABS " + debugText})
...
func (grid *SelectiveGridStatWeightProcess) reportOutputWeightsGrid(solution *highs.Solution) WeightResult {
	...
	weightValues := util.MapMap_FromExitingMapMap_WithApplyPlusKeys(&grid.detailedWeights,
		func(statType stats.StatType, _ stats.SimType, weightCol util_highs.ColumnIndex) float64 {
	...
	for colIndex := range solution.ColValues {
		debug := grid.build.DebugTextFor(util_highs.ColumnIndex(colIndex))
