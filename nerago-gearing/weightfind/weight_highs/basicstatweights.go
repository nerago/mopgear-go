...
func (basic *BasicStatWeightProcess) Init(printer *util.PrintRecorder) {
	...
	basic.build.Solver = util_highs.Solver_LP_NO_GPU // needs sync and would be overkill
	basic.simData = make(map[stats.StatType]basicDataEntry)
	basic.finalWeights = make(map[stats.StatType]util_highs.ColumnIndex)
}
...
func (basic *BasicStatWeightProcess) Run(stopwatch *util.Stopwatch) *util_async.FutureCancellable[weight_types.WeightBasic] {
...
func (basic *BasicStatWeightProcess) unitValuesToCalcDetailedRatings_single(unitValueBase float64, detailWeightBase util_highs.ColumnIndex,
...
func (basic *BasicStatWeightProcess) reportOutputWeights(solution *highs.Solution) weight_types.WeightBasic {
