...
func arrayRankToSetRangeStatisticalComplicated[T weight_types.IRankEntry](data []T, getSimScore func(T) averageAndStdDev, setSimRankRange func(T, *util.HiLoInt)) {
...
func arrayApplyChainFunc[T weight_types.IRankEntry](data []T, runStart int, countChainedRun int, countEqualFirstRun int, setSimRankRange func(T, *util.HiLoInt)) int {
	if countChainedRun == 1 {
		setSimRankRange(data[runStart], &util.HiLoInt{Lo: runStart, Hi: runStart})
		return runStart + 1
	} else {
		var end int
